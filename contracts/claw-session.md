...
## Config
Owns the per-session DB schema-version declarations (current inbound/outbound versions and the runner protocol version). The host supplies the derived sessions root (read from claw-config) when deriving a session layout; archive policy is a later milestone.
