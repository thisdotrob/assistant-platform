...
(see Persistence): `evaluate_sender` (claw-permissions) under the configured
`UnknownPolicy` — an unknown sender is dropped and audited — then
`evaluate_engagement` (claw-router) under the configured `EngagementMode`; an
...
occurrence (claw-scheduler) and drives each as a turn into the occurrence's target
...
of `SpecialistSpec`s (the `claw-specialist-spec` vocab — plain data describing a
...
text. The agent-graph engine (claw-agent-graph) owns the job lifecycle, policy,
...
(`claw_agent_graph::store::add_run_link`), additive provenance whose failure is
...
implements claw-memory's pure `MemoryBackend` seam by shelling the host-side
...
empty degraded) into `MemoryHealth`, and search replies reuse claw-memory's
`parse_qmd_response`. The host is the only crate allowed to shell out, which is why
this lives here rather than in claw-memory; failure modes (missing binary, timeout,
...
is the hex encoding of the id (`claw_memory::corpus_stem_for_memory_id`), a fixed
...
only through the `claw-session` public API (host writes inbound even-seq, reads
outbound read-only). A `Host`'s delivered high-water mark is held in memory but
resumed from the persisted `delivered` marker (`claw-session::max_delivered_seq`)
...
owned by `claw-router`, `claw-permissions`, `claw-scheduler`, `claw-memory`, and
`claw-agent-graph`) on top of the M1 baseline, so a real install has the
...
agent-group state only through claw-agent-graph's engine/store API (`create_specialist`,
...
no SQL): it reads `user_dms` (claw-permissions `evaluate_sender`) and the active
`sticky_engagement` row (claw-router `has_active_sticky`), and writes the
`dropped_messages` audit (claw-router `record_drop`) for a rejected message and a
`sticky_engagement` row (claw-router `open_sticky`) when an engaging message
opens/refreshes the sticky window. The `register-user` admin action writes
`users`/`user_roles`/`user_dms` via claw-permissions (`bootstrap_owner` /
...
central DB read-only and reads the `memory_entries` catalog through claw-memory
...
claw-memory is retained as latent capability, not wired here.
...
(`claw-memory inject_from_search` → `render_memory_block`) — snippets come straight
...
content-stably via claw-memory `generate_memory_id`), and projects its front matter
into the `memory_entries` catalog via claw-memory `upsert_entry` (authoring no SQL),
...
(claw-router `expire_sticky`), then over `scheduled_items`/`scheduled_occurrences`
it claims every due occurrence (claw-scheduler `claim_due`, which writes the lease
...
`claw-config` and the session id/agent group from `RunOptions`. Both run paths
...
runtime), and `now` (epoch seconds). claw-scheduler never reads the wall clock, so
...
## Readiness Checks
Reuses `claw-runtime-docker` readiness (Docker daemon reachable, base image
