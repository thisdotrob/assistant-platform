...
contract for that recovery — the corpus writer (in `assistant-host`) names each file by
...
actually shells qmd (a node sidecar) lives in `assistant-host`, not here, so this crate
...
## Config
Reads memory root paths and reindex cadence from assistant-config; the product supplies the memory taxonomy.
