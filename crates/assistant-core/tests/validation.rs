...
fn codes(diags: &[assistant_core::Diagnostic]) -> Vec<DiagnosticCode> {
    diags.iter().map(|d| d.code).collect()
}

fn platform(modules_toml: &str) -> assistant_core::PlatformManifest {
    let src = format!(
        r#"
...
[[fragment]]
id = "safety"
owner_module = "assistant-agent-protocol"
version = "0.1.0"
...
"#,
    ...
    let known: BTreeSet<&str> = ["assistant-agent-protocol"].into_iter().collect();
    let mut diags = Vec::new();
...
fn missing_contract_file_fails() {
    ...
    contracts.insert("assistant-core".to_string(), full_coverage());
    let mut diags = Vec::new();
    check_contract_presence(&["assistant-core", "assistant-db"], &contracts, &mut diags);
...
fn contract_missing_required_section_fails() {
    ...
    contracts.insert("assistant-core".to_string(), coverage);
    let mut diags = Vec::new();
    check_contract_presence(&["assistant-core"], &contracts, &mut diags);
...
enabled_modules = ["assistant-core"]
"#,
    )
