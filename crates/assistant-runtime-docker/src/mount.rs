...
//! session layout (in `assistant-session`) and supplies resolved paths here, so this
//! crate stays free of a `assistant-session` dependency. Mounts are validated
...
/// a `assistant-session` `SessionLayout`.
