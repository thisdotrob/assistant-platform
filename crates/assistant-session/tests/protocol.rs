...
fn old_v1_fixture_migrates_lazily_on_open() {
    ...
    {
        let mut conn = assistant_session::db::open_read_write(&layout.inbound_db_path()).unwrap();
    ...
    let conn = assistant_session::open_inbound(&layout).unwrap();
...
fn old_v1_outbound_fixture_migrates_lazily_on_recovery() {
    ...
    std::fs::create_dir_all(layout.dir()).unwrap();
    {
        let mut conn = assistant_session::db::open_read_write(&layout.outbound_db_path()).unwrap();
