...
//! or Claude. The sink is backed by `assistant-session`'s `FakeContainer`, which is a
//! dev-dependency only — production `assistant-agent-protocol` depends on `assistant-core`
//! alone.

use assistant_agent_protocol::{
    run_once, AgentKind, Delivery, FallbackLedger, OutboundAction, OutboundSink, RunContext,
    RunResult, StubProvider,
};
use assistant_agent_protocol::fragment::shared_fragments;
use assistant_session::{current_outbound_compat, read_outbound, FakeContainer, LocalControl, SessionLayout};
