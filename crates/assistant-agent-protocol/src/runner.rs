...
//! abstraction so this crate never depends on `assistant-session` in production —
//! the session-backed sink lives in tests as a dev-dependency. Keeping the loop
//! behind the sink trait is what lets `assistant-agent-protocol` stay
//! `depends_on = ["assistant-core"]` in the platform manifest.
