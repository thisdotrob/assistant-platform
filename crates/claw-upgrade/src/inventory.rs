...
//! no tables of its own — it reads claw-db's meta tables through claw-db's
//! public API and each session DB's `schema_meta` through claw-session's.

use std::fs::{self, ReadDir};
use std::path::Path;

use claw_config::InstanceLayout;
use claw_db::VersionRecord;
use claw_session::{db as session_db, schema_version, DbKind, SessionLayout};
...
pub fn inventory(layout: &InstanceLayout) -> Result<InstanceInventory, UpgradeError> {
    ...
    let (central_present, recorded, applied_central_versions) = if central_path.exists() {
        ...
        let recorded = claw_db::read_version_record(&conn)?;
        let applied = claw_db::applied_versions(&conn)?;
...
mod tests {
    use super::*;
    use claw_db::{apply, record_versions, Migration, MigrationSet, VersionRecord};
    use claw_session::{lazy_migrate, migrations_for, migrations_v1_only};
