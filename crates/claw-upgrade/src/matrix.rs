...
//! version) is delegated to [`claw_core::run_compatibility_check`]. On top of
...
use claw_core::{CompatibilityReport, PlatformManifest, ProductManifest};
use claw_db::{MigrationSet, VersionRecord};
use claw_session::{CURRENT_INBOUND_VERSION, CURRENT_OUTBOUND_VERSION};
...
pub fn compatibility_matrix(
    ...
    let repo = claw_core::run_compatibility_check(platform_root, product_root)?;
    let platform = claw_core::compat::load_platform_manifest(platform_root)?;
    let product = claw_core::compat::load_product_manifest(product_root)?;
...
";
    ...
    fn conformance_is_clean_on_a_fresh_instance() {
        ...
        claw_db::apply(&mut conn, &set).unwrap();
