...
//! [`claw_session::SessionLayout`], which forbids identifiers that could escape
...
use claw_db::{MigrationSet, VersionRecord};
use claw_session::{db as session_db, lazy_migrate, migrations_for, schema_version, DbKind};
...
fn apply_central(
    ...
    let mut conn = claw_db::open_central(central_path)?;
    let report = claw_db::apply(&mut conn, set)?;
    claw_db::record_versions(&conn, record)?;
...
fn plan_central(
    ...
    let conn = claw_db::open_central(central_path)?;
    let recorded_max: BTreeMap<String, u32> =
        claw_db::applied_versions(&conn)?.into_iter().collect();
...
fn sweep_db(
    root: &Path,
    session_layout: &claw_session::SessionLayout,
    kind: DbKind,
    dry_run: bool,
...
mod tests {
    use super::*;
    use claw_session::{lazy_migrate, migrations_v1_only, SessionLayout};
    use std::fs;
    ...
    fn central_set(versions: &[u32]) -> MigrationSet {
        let mut set = MigrationSet::new(vec!["claw-db".to_string()]);
        for &v in versions {
            ...
            set.add(claw_db::Migration::new("claw-db", v, name, sql));
        }
        set
    }
    ...
    fn version_record() -> VersionRecord {
        VersionRecord {
            ...
            modules: vec![("claw-db".to_string(), "0.1.0".to_string())],
        }
    }

    /// Build an instance whose central DB is at claw-db v1.
    fn build_v1_instance(layout: &InstanceLayout) {
        fs::create_dir_all(&layout.root).unwrap();
        let mut conn = claw_db::open_central(&layout.central_db_path()).unwrap();
        claw_db::apply(&mut conn, &central_set(&[1])).unwrap();
        claw_db::record_versions(&conn, &version_record()).unwrap();
    }
    ...
    fn upgrade_from_baseline_then_idempotent_rerun() {
        ...
        assert_eq!(report.central_applied, vec![("claw-db".to_string(), 2)]);
        assert_eq!(report.central_skipped, vec![("claw-db".to_string(), 1)]);
        ...
        let conn = claw_db::open_central(&layout.central_db_path()).unwrap();
        assert_eq!(
            claw_db::applied_versions(&conn).unwrap(),
            vec![("claw-db".to_string(), 2)]
        );
        drop(conn);
        ...
        assert_eq!(
            ...
            vec![("claw-db".to_string(), 1), ("claw-db".to_string(), 2)]
        );
