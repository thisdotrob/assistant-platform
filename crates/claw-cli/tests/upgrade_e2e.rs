...
//! old v1 schema — and then driven through the public `claw_cli::upgrade`. The
...
use claw_cli::{upgrade, BootstrapRequest};
...
fn upgrade_applies_central_baseline_and_advances_sessions_then_is_idempotent() {
    ...
    // Central: the full baseline was applied (claw-session's owner migration
    // among them).
    let conn = claw_db::open_central(&layout.central_db_path()).unwrap();
    let applied = claw_db::applied_versions(&conn).unwrap();
    drop(conn);
    assert!(!applied.is_empty(), "central baseline applied");
    assert!(
        applied.iter().any(|(m, v)| m == "claw-session" && *v == 1),
        "claw-session baseline recorded: {applied:?}"
    );
...
fn dry_run_reports_work_but_writes_nothing() {
    ...
    let conn = claw_db::open_central(&layout.central_db_path()).unwrap();
    assert!(
        claw_db::applied_versions(&conn).unwrap().is_empty(),
