...
use claw_channel_telegram::{normalize, parse_update, TelegramIdentity};
use claw_memory::{Scope, SearchOutcome};
use claw_permissions::{AccessDecision, UnknownPolicy};
use claw_router::{
    EngagementContext, EngagementDecision, EngagementMode, IgnoredMessagePolicy, SenderScope,
};
use claw_web::{SessionView, WebApp};
...
fn engage(event: &claw_router::RoutingEvent, sender_is_member: bool) -> EngagementDecision {
    claw_router::evaluate_engagement(
        event,
