...
//! reindex loop upserts a row per entry). assistant-memory already owns the baseline
...
/// assistant-memory's central-DB migrations beyond the baseline `memory_health` (v1).
