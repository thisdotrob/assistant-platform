...
//! render richly). It speaks to a session entirely through the `claw-session`
//! public API; it owns no session-DB schema of its own.
//!
//! It conforms to [`claw_router::ChannelAdapter`] so the host can drive it
//! uniformly alongside Slack/Telegram, and normalizes raw terminal input into a
//! [`claw_router::RoutingEvent`] for the router.
...
pub const MODULE_ID: &str = "claw-channel-cli";
