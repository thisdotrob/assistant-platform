...
mod tests {
    ...
    fn only_enabled_modules_get_checks() {
        let surface = EnabledSurface {
            modules: vec!["assistant-core".to_string(), "assistant-session".to_string()],
            profile: Some(("assistant".to_string(), "0.1.0".to_string())),
            capabilities: vec![],
        };
        let registry = ReadinessRegistry::for_enabled(&surface);
        assert!(registry.contains("module:assistant-core"));
        assert!(registry.contains("module:assistant-session"));
        assert!(registry.contains("profile:assistant@0.1.0"));
        // A module that is not enabled has no check.
        assert!(!registry.contains("module:assistant-channel-slack"));
    ...
    fn record_unknown_check_is_rejected() {
        let mut registry = ReadinessRegistry::for_enabled(&EnabledSurface {
            modules: vec!["assistant-core".to_string()],
            profile: None,
            capabilities: vec![],
        });
        assert!(registry.record("module:assistant-core", CheckStatus::Pass, None));
