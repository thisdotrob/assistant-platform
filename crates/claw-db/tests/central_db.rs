...
use claw_db::schema::baseline_owner_modules;
use claw_db::{
...
fn version_recording_persists() {
    ...
    let recorded: String = conn
        .query_row(
            "SELECT platform_version FROM module_versions WHERE module_id = 'claw-core'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn checksum_drift_refuses_on_reopen() {
    ...
    {
        let mut conn = open_central(&db_path).unwrap();
        let mut set = MigrationSet::new(vec!["claw-core".to_string()]);
