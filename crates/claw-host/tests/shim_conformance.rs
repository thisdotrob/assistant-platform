...
//!   cargo test -p claw-host --test shim_conformance -- --ignored --nocapture

use std::path::PathBuf;
use std::process::{Child, Command, Stdio};
use std::time::Duration;

use claw_host::{Host, HostConfig};
use claw_runtime_docker::{FakeRuntime, ImageRef, OneCliReadiness, RunnerAuthMode};
use claw_session::{init_session, verify_sequence_parity, InboundMessage, SessionLayout};
...
fn real_node_shim_round_trips_a_turn() {
    ...
    let mut config =
        HostConfig::new(ImageRef::new("claw-agent-base", "0.1.0"), vec![sessions], RunnerAuthMode::Stub, ready());
