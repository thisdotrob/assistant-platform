...
use claw_runtime_docker::{FakeRuntime, ImageRef, OneCliReadiness, RunnerAuthMode};
use claw_session::{
...
fn test_config(sessions: PathBuf) -> HostConfig {
    let mut config = HostConfig::new(
        ImageRef::new("claw-agent-base", "0.1.0"),
        vec![sessions],
        RunnerAuthMode::Stub,
        ready(),
    )
...
/// Create and migrate a central DB (baseline + claw-router's sticky/drops) at
...
fn migrate_central(path: &Path) {
    ...
    for migration in claw_router::migrations() {
        set.add(migration);
    }
    // The scheduler's projection/occurrence tables, as setup_steps applies them.
    for migration in claw_scheduler::migrations() {
        set.add(migration);
    }
...
fn seed_due_item(
    ...
    // Agent group 1 matches claw-host's single-host HOST_AGENT_GROUP.
...
fn a_turns_save_memory_action_is_written_and_surfaced_in_a_later_turn() {
    ...
    let conn = open_central(&central).unwrap();
    let rows = claw_memory::entries_for_agent(&conn, 1).unwrap();
    ...
    assert_eq!(source_ref.channel, Some(claw_memory::SourceChannel::Slack));
