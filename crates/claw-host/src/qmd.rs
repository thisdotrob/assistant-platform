...
//!
//! `claw-memory` stays pure (it owns [`parse_qmd_response`] and the trait); the
...
impl<R: SidecarRunner> NodeQmd<R> {
    ...
    fn write_corpus(&self, docs: &[IndexDoc]) -> std::io::Result<()> {
        ...
        for doc in docs {
            ...
            let stem = claw_memory::corpus_stem_for_memory_id(&doc.memory_id);
...
mod tests {
    ...
    fn search_parses_runner_hits() {
        let backend = NodeQmd::with_runner(
            ...
            |sub: &str, _stdin: &str, _t: Duration| {
                ...
                let stem = claw_memory::corpus_stem_for_memory_id("mem_a");
                Ok(format!(
    ...
    fn reindex_writes_corpus_and_reports_health() {
        ...
        let a = claw_memory::corpus_stem_for_memory_id("mem_a");
        let b = claw_memory::corpus_stem_for_memory_id("mem_b");
    ...
    fn reindex_clears_stale_corpus_files() {
        ...
        let fresh = claw_memory::corpus_stem_for_memory_id("fresh");
