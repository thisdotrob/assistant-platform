...
//! [`claw_agent_protocol::OutboundAction::SaveMemory`]) and the host writes the
...
/// `content` (the serde body of [`claw_agent_protocol::OutboundAction::SaveMemory`]
...
mod tests {
    ...
    use claw_memory::{entries_for_agent, retrieve, RetrievalContext, SourceChannel};

    fn catalog_db() -> Connection {
        let mut conn = Connection::open_in_memory().unwrap();
        let mut set = MigrationSet::new(vec![claw_memory::MODULE_ID.to_string()]);
