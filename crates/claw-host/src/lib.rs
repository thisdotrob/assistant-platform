...
//!
//! `claw-cli` stays a domain-free framework and the channel crate stays out of
...
use claw_runtime_docker::{BASE_IMAGE_REPOSITORY, DockerCliRuntime, ImageRef};
...
/// dependency on `claw-runtime-docker`.
pub use claw_runtime_docker::RunnerAuthMode;

/// Re-exported so products can register specialists (and tests can build specs)
/// without taking a direct dependency on `claw-specialist-spec`.
pub use claw_specialist_spec::SpecialistSpec;

/// Re-exported so products and tests can set the Slack serve gate without taking
/// direct dependencies on `claw-router`/`claw-permissions`.
pub use claw_permissions::UnknownPolicy;
pub use claw_router::EngagementMode;

pub const MODULE_ID: &str = "claw-host";
...
fn qmd_sidecar_config(layout: &InstanceLayout) -> Option<qmd::QmdSidecar> {
    ...
    let qmd_dir = claw_memory::MemoryRoot::orchestrator(&layout.groups_dir(), HOST_AGENT_OWNER)
        .path()
        .join("qmd");
    let mut sidecar =
...
fn spawn_qmd_reindex(layout: &InstanceLayout, sidecar: qmd::QmdSidecar) {
    ...
    std::thread::spawn(move || {
        let conn = match claw_db::open_central(&central_db_path) {
            Ok(conn) => conn,
            Err(e) => {
        ...
        let root = claw_memory::MemoryRoot::orchestrator(&groups_dir, HOST_AGENT_OWNER);
        let mut backend = qmd::NodeQmd::new(sidecar);
        match claw_memory::reindex_root(&conn, &root, HOST_AGENT_GROUP, &mut backend) {
            Ok(report) => eprintln!(
