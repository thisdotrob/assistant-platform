...
//!
//! Both products append [`setup_steps`] to their `claw_cli::setup` call so they
...
//! hard [`claw_setup::readiness_gate`] last (deferred here while per-module
//! readiness reporting is still partial).

use std::process::Command;

use claw_db::{apply, open_central, Migration, MigrationSet};
use claw_runtime_docker::BASE_IMAGE_REPOSITORY;
use claw_setup::{CheckStatus, FnStep, SetupStep};
...
/// `claw-agent-base:<platform version>`.
...
fn domain_migrations() -> MigrationSet {
    let mut set = MigrationSet::new(vec![
        ...
        claw_permissions::MODULE_ID.to_string(),
        claw_scheduler::MODULE_ID.to_string(),
        claw_memory::MODULE_ID.to_string(),
        claw_agent_graph::MODULE_ID.to_string(),
    ]);
    let v2s: Vec<Migration> = claw_router::migrations()
        .into_iter()
        .chain(claw_permissions::migrations())
        .chain(claw_scheduler::migrations())
        .chain(claw_memory::migrations())
        .chain(claw_agent_graph::store::migrations())
        .collect();
...
/// Build the shared `claw-agent-base` image from a `Dockerfile` context. Runs
...
fn build_agent_image() -> Box<dyn SetupStep> {
    FnStep::new(
        "build_agent_image",
        "Build the claw-agent-base agent container image",
        |ctx| {
            ...
            let output = Command::new("docker")
                ...
                .map_err(|source| claw_setup::SetupError::Io {
                    path: std::path::PathBuf::from(&dir),
                    source,
                })?;
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(claw_setup::SetupError::Io {
                    path: std::path::PathBuf::from(&dir),
...
fn configure_onecli() -> Box<dyn SetupStep> {
    FnStep::new(
        ...
        |ctx| {
            ...
            ctx.record_readiness("module:claw-host", status, Some(detail.clone()));
            Ok(detail)
        },
    )
    .boxed()
...
mod tests {
    ...
    fn domain_migrations_layer_v2_on_baseline() {
        ...
        for module in [
            claw_router::MODULE_ID,
            claw_permissions::MODULE_ID,
            claw_scheduler::MODULE_ID,
            claw_memory::MODULE_ID,
            claw_agent_graph::MODULE_ID,
        ] {
            assert!(
    ...
    fn prepared() -> (tempfile::TempDir, InstanceLayout) {
        ...
        let surface = EnabledSurface {
            modules: vec!["claw-host".to_string()],
            profile: None,
    ...
    fn migrate_domain_db_step_applies_to_central_db() {
        ...
        let versions = applied_versions(&conn).unwrap();
        assert!(versions.contains(&(claw_memory::MODULE_ID.to_string(), 2)));
        assert!(versions.contains(&(claw_permissions::MODULE_ID.to_string(), 2)));
        assert!(versions.contains(&(claw_scheduler::MODULE_ID.to_string(), 2)));
        assert!(versions.contains(&(claw_router::MODULE_ID.to_string(), 2)));
        assert!(versions.contains(&(claw_agent_graph::MODULE_ID.to_string(), 2)));
    }
}
