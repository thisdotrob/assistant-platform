...
//! `outbound.db` read-only, all through the `claw-session` public API.

use std::io::Write;
use std::path::PathBuf;
use std::time::{Duration, Instant};

use claw_agent_protocol::{check_runner_protocol, RUNNER_PROTOCOL_VERSION};
use claw_channel_cli::{local_sender, render_message};
...
pub struct HostConfig {
    /// Base agent image to spawn (`claw-agent-base:<version>`).
    pub image: ImageRef,
    ...
    R::Error: std::fmt::Display,
{
    ...
    fn memory_block(&self, inbound: &InboundMessage) -> Option<String> {
        ...
        // `claw_memory` is kept as latent capability, not a TODO to finish here.
        let envelopes =
