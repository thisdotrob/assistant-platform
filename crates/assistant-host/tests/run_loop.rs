...
use assistant_host::{sweep_once, Host, HostConfig, HostError, SweepReport};
...
fn test_config(roots: Vec<PathBuf>, mode: RunnerAuthMode, onecli: OneCliReadiness) -> HostConfig {
    let mut config = HostConfig::new(ImageRef::new("assistant-base", "0.1.0"), roots, mode, onecli)
...
fn catalog_memory_is_injected_into_inbound_metadata() {
    ...
    // Migrate the central DB to the baseline (assistant-memory v1 included) plus the
    ...
    // connection before the turn runs so the host's own read is uncontended.
    {
        ...
        let mut catalog = MigrationSet::new(vec![assistant_memory::MODULE_ID.to_string()]);
...
fn scheduler_sweep_fires_due_item_once_and_expires_sticky() {
    ...
    // already-expired sticky window the sweep should clear.
    {
        ...
        for migration in assistant_router::migrations().into_iter().chain(assistant_scheduler::migrations()) {
            domain.add(migration);
        }
        apply(&mut conn, &domain).unwrap();
