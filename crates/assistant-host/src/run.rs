    ...
    R::Error: std::fmt::Display,
{
    ...
    fn ensure_spawned(&mut self) -> Result<(), HostError> {
        ...
        if matches!(
            ...
            // API-key mode: the container authenticates with
            // ANTHROPIC_API_KEY=placeholder (sent as x-api-key), which OneCLI
            // rewrites to the real Anthropic API key on outbound traffic. Both
            // prepare_runner_env and OneCLI's container-config set that
            // placeholder; we deliberately keep it — and leave
            // CLAUDE_CODE_OAUTH_TOKEN cleared — so Claude Code uses the API-key
