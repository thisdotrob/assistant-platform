...
use assistant_agent_protocol::UnsupportedProtocol;
use assistant_runtime_docker::SpawnError;
...
pub enum HostError {
    ...
    Layout(String),
    /// A `assistant-session` operation failed.
    ...
    /// `HostError` stays free of the `assistant-db`/`assistant-permissions` error types.
    Db(String),
