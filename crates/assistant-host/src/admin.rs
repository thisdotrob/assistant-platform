...
//!
//! The Slack serve path is deny-by-default ([`assistant_permissions::UnknownPolicy::Strict`]):
...
mod tests {
    ...
    use assistant_permissions::{resolve_user_by_dm, UnknownPolicy};
    use std::path::Path;
    ...
    fn registering_binds_a_dm_route_that_resolves() {
        ...
        assert!(
            assistant_permissions::evaluate_sender(&conn, "slack", "U_ROB", UnknownPolicy::Strict)
                .unwrap()
                .is_allow()
        );
    }
