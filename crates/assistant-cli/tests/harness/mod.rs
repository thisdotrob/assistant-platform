...
//! host would drive it.
//!
//! One faithful detail this surfaces: `assistant_cli`'s bootstrap applies only the
...
//! bootstrapped DB — `assistant_db::apply` is idempotent, so the baseline rows are
...
pub fn platform_version() -> String {
    assistant_core::platform_metadata().version.to_string()
}
