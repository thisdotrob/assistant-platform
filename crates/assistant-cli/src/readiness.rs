...
/// The aggregated readiness of every registered module. assistant-cli registers no
...
mod tests {
    ...
    fn report() -> ReadinessReport {
        ...
        r.add("assistant-db", "schema_current", CheckState::Pass);
        r.add("assistant-session", "parity", CheckState::Skipped { detail: "no sessions".to_string() });
        r.add("assistant-router", "channels_connected", CheckState::Fail { detail: "slack down".to_string() });
        r
    }
    ...
    fn skipped_does_not_block_but_fail_does() {
        ...
        clean.add("assistant-db", "schema_current", CheckState::Pass);
        clean.add("assistant-session", "parity", CheckState::Skipped { detail: "n/a".to_string() });
    ...
    fn table_render_shows_every_check_and_the_failure_summary() {
        ...
        assert!(rendered.contains("assistant-db"));
