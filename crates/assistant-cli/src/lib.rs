...
use assistant_config::{Config, InstanceLayout, ModulesConfig, ProductConfig, WebConfig};
use assistant_core::platform_metadata;
use assistant_db::{baseline_migrations, VersionRecord};
use assistant_setup::{
    run, BootstrapInput, BootstrapOptions, PipelineOptions, ProgressSink, SetupPipeline, SetupStep,
};
use assistant_upgrade::{
...
// Re-exported so a product can assemble its setup pipeline against `assistant-cli`
// alone, without taking a direct dependency on `assistant-setup`.
pub use assistant_setup::{
    readiness_gate, CheckStatus, FnStep, SetupContext, SetupError, SetupRun,
};

pub const MODULE_ID: &str = "assistant-cli";
...
/// strings, so products stay decoupled from `assistant-upgrade`'s own types. Returns
...
pub fn conformance(
    ...
    match assistant_upgrade::conformance(
        platform_root,
        product_root,
        &layout,
        &runtime,
