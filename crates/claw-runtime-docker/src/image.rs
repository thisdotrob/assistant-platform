//! Docker image naming and the `claw-agent-base` image contract.
//!
//! Product images layer on a shared `claw-agent-base` image; this module names
...
pub const BASE_IMAGE_REPOSITORY: &str = "claw-agent-base";
...
mod tests {
    ...
    fn tag_reference_when_unpinned() {
        let image = ImageRef::new("claw-agent-base", "0.1.0");
        assert_eq!(image.reference(), "claw-agent-base:0.1.0");
        assert!(!image.is_pinned());
    }

    #[test]
    fn digest_takes_precedence_when_pinned() {
        let image = ImageRef::pinned("claw-agent-base", "0.1.0", "sha256:abc123");
        assert_eq!(image.reference(), "claw-agent-base@sha256:abc123");
        assert!(image.is_pinned());
    }
    ...
    fn base_contract_uses_platform_version_as_tag() {
        ...
        assert_eq!(contract.image.reference(), "claw-agent-base:0.1.0");
