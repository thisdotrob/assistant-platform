...
//!
//! claw-approvals owns the wording that tells the orchestrator when to stop and
//! request human approval, and how an approval card is rendered and matched back
//! to its response. Like every owning crate, claw-approvals exposes only the
//! fragment's id, order, parameters, and body — it does NOT depend on
//! claw-agent-protocol; the assembler wraps these. The manifest entry is the
...
/// replacement keeps the crate free of a dependency on claw-agent-protocol's
