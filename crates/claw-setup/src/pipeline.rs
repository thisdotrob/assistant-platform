...
mod tests {
    ...
    fn dry_run_writes_nothing_and_reports_every_step() {
        let (_h, layout) = prepared(&["claw-core"]);
    ...
    fn readiness_gate_blocks_until_all_checks_pass() {
        let (_h, layout) = prepared(&["claw-core", "claw-session"]);
        // A step that only marks one of the two checks passing, then the gate.
        let mark_core = FnStep::new("mark_core", "pass claw-core", |ctx| {
            assert!(ctx.record_readiness("module:claw-core", CheckStatus::Pass, None));
        ...
        let err = pipeline
            ...
            .unwrap_err();
        match err {
            SetupError::Gate { detail, .. } => assert!(detail.contains("module:claw-session")),
            other => panic!("expected gate error, got {other}"),
        }

        // Mark the remaining check and resume: the gate now passes.
        let mark_session = FnStep::new("mark_session", "pass claw-session", |ctx| {
            assert!(ctx.record_readiness("module:claw-session", CheckStatus::Pass, None));
            Ok("recorded".to_string())
        })
        .boxed();
        let pipeline2 = SetupPipeline::new()
