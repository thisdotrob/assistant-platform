...
use claw_config::{Config, ModulesConfig, ProductConfig, WebConfig};
use claw_db::{baseline_migrations, baseline_owner_modules, Migration, MigrationSet, VersionRecord};
use claw_setup::{run, BootstrapInput, BootstrapOptions, SetupError, StepId};
...
fn sample_config() -> Config {
    Config {
        ...
        modules: ModulesConfig {
            enabled: vec!["claw-core".to_string()],
        },
...
fn version_record() -> VersionRecord {
    VersionRecord {
        ...
        product_version: "0.1.0".to_string(),
        platform_version: "0.1.0".to_string(),
        modules: vec![("claw-core".to_string(), "0.1.0".to_string())],
    }
}
...
fn checksum_mismatch_refuses_on_resume() {
    ...
    // Initial migration set defines claw-core v1.
    let mut original = MigrationSet::new(vec!["claw-core".to_string()]);
    ...
    let mut drifted = MigrationSet::new(vec!["claw-core".to_string()]);
